...
class FisherDiscriminant(Classifier):
    def __init__(self, X_train, X_test, y_train, y_test, loss):
        ...
        self.classifier = OneVsRestClassifier(LinearDiscriminantAnalysis(solver='svd', shrinkage=None, priors=None, n_components=None, store_covariance=False, tol=0.0001))#LinearSVC(loss=self.loss, verbose=True, tol=0.001))
        self.kfolded = False
